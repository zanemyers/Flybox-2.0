...
- `src/server/rateLimit.ts` — per-client (3/hour, 10/day) and global (40/day, 200/30 days) caps, counted from `RunLedger` and recorded in the same locked transaction that admits the run. The client is identified by a salted SHA-256 of its IP
...
- `src/server/net.ts` — an address guard on every outbound fetch. The crawler's targets are chosen by third parties, and the crawled bytes are published on `/runs`. A listed site could therefore redirect to `http://169.254.169.254/` or a loopback port and have the response served to anyone. Non-HTTP schemes, local-only names and any host resolving outside public space are refused; `httpFetch` now follows redirects hop by hop with a check on each, and Playwright's own redirects are checked in its route handler
- A cap on downloads — 60 per client per minute, held in memory. `/runs` publishes the job ids that address the file route, and each hit reads a blob out of Postgres. The 2s poll also reads at most 500 log lines, where it previously had no ceiling
...
- `Job` columns for what a run was for: `latitude`, `longitude`, `locationName`, `rivers`, `summarized`. Nothing identifying — the rate limiter's IP hash lives on `RunLedger` alone, since a `Job` outlives every window a cap counts over
- `report_raw.txt` (`Job.rawFile`) — the crawled source text, kept on summarized runs so one can still offer what it was built from. Raw mode does not write it, since `report_summary.txt` already is that text there
...
- Tests for the defects fixed below. That the rate-limit retention window outlives every window a cap counts over. That `httpFetch` refuses non-markup and caps its reads. That robots.txt is fetched once per origin rather than once per page
...
- The shop directory is now an option on the form. The shop phase still runs either way, because the report phase needs its `fishingReport` flags. The toggle only decides whether the workbook is built and stored
- The updates endpoint reports an `expected` manifest, and readiness only for what is on it. Three files used to auto-download, including `report_raw.txt`. That one had no row in the panel, and Chrome was likely blocking it as a repeat automatic download
...
- The robots.txt cache holds parsed rules rather than raw text. It is bounded at 500 origins, expires after 6h, and collapses concurrent misses on one origin into a single request
...
- The status panel's output metadata is exhaustive over `OutputName`. Adding an output is now a compile error there, rather than a row that throws on render
- `FormState` in the form is `Payload` rather than a structural twin that had to be kept in step with it
- The 404's right column is a sounder readout with no reading — dashes, not zeroes. It reuses the `.well` list from `/how-it-works`, so `brand.tsx` stays at two drawings
...
### Fixed
- Every rate limit was uncountable. The caps counted `Job` rows, which retention deletes on the catalog's schedule — failed and canceled outright, completed past the newest 15. Each window therefore shortened to whatever survived the last prune, and with cleanup on a cron the 200-run monthly cap could never be reached. Counts now come from `RunLedger`, pruned on its own window. `RATE_LIMIT_WINDOW_MS` is the single constant both the count and the prune read, so evidence cannot again outlive its claim
- The caps were also bypassable by a parallel burst. `checkRateLimit` counted and `JobHandler.create` inserted with nothing in between. Concurrent requests therefore all read the same pre-insert totals and all passed: 25 at once against a 3/hour cap took 25 runs. Admission is now one transaction holding `pg_advisory_xact_lock`, and takes exactly 3
...
- The non-DaisyUI tokens (`--stroke`, `--rule`, `--sunken`, `--mark`, the body gradient) were declared only under `[data-theme]`. With no attribute they resolved empty, so every hairline fell back to `currentColor` — the whole page outlined in full-strength ink. They now sit on `:root`. `prefersdark` came off the dark theme in the same change. It applied DaisyUI's dark colors under a media query while those tokens stayed light. That is how a coherent page became a cream well on a dark panel. The theme script already reads the OS preference before first paint, so it was only ever deciding the no-JS render
- The map dialog had no accessible name, announcing as an unnamed dialog; it is now labeled by its visible heading
...
- The per-client rate limit was bypassable. `clientHashFrom` read the leftmost `x-forwarded-for` entry, which is whatever the caller sent. Rotating that header gave a fresh identity per request, leaving only the global caps. One client could burn the daily allowance and lock everyone else out. The address is now taken by counting in from the right by `RATE_LIMIT_TRUSTED_PROXIES` (default 1). A count that is too low is detected by the selected address being in a private range
...
- robots.txt parsing: directive names are lowercased but values are not, since rule paths are case-sensitive. Also `*` wildcards, the `$` anchor, inline `#` comments, and consecutive `User-agent` lines as one group
...
- A run interrupted mid-flight (a deployment, a crash) stayed `IN_PROGRESS` forever: the client polled it indefinitely and no cleanup path would ever match it. Runs now stamp `Job.heartbeatAt` on the cancel check they were already making; a frozen stamp means the run is abandoned, so the next poll reports `FAILED` and `db_cleanup` deletes it
- `Job.clientHash` is cleared once it can no longer affect a limit. The ledger's own hash clears on the same window, while its row survives to be counted
- The theme script is an inline `<script>` in `<head>`, not `next/script` with `beforeInteractive`. That was queued into `self.__next_s` and flashed the light theme on every load
