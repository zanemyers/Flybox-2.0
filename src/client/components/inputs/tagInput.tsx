...
export default function TagInput({ label, values, onChange, placeholder, optional = false }: {
    label: string;
    values: string[];
    ...
    return (
        <div className="w-full">
            <label className="input-label">
                {label}{optional && <span className="text-base-content/40 font-normal"> (optional)</span>}
            </label>
            <div
            ...
            >
                {values.map((v) => (
                    <span key={v} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-primary/15 text-primary text-sm font-medium">
                        {v}
                        <button type="button" onClick={() => remove(v)} className="leading-none opacity-60 hover:opacity-100">×</button>
                    </span>
                ))}
                <input
                    type="text"
