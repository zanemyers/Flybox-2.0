...
export default function TextareaInput({ label, value, onChange, defaultValue = "" }: {
    label: string;
    value: string;
    ...
    const open = () => { setDraft(value); setShow(true); };
    const save = () => { onChange(draft); setShow(false); };

    return (
        <>
            <div className="w-full">
                <label className="input-label">{label} <span className="text-error">*</span></label>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center">
                    ...
                    <div ref={modalRef} className="relative bg-base-100 rounded-box shadow-xl w-[90%] max-w-2xl p-6 flex flex-col gap-4">
                        ...
                        <div className="flex justify-between">
                            {defaultValue && (
                                <button type="button" className="btn btn-ghost btn-sm" onClick={() => setDraft(defaultValue)}>Reset to Default</button>
                            )}
