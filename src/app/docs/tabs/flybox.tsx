...
import { simpleDetails, details } from "@/client/images/docs";
import { DocSection, ListBlock } from "@/client/components/docs";
import type { TocItem, ListItems } from "@/server/types/componentTypes";
...
export default function FlyboxDoc() {
    return (
        <>
            ...
            <p>
                Flybox finds fly-fishing shops using <strong>Google Maps</strong> via <strong>SerpAPI</strong>,
                scrapes their websites for fishing reports, and summarizes them with <strong>Google Gemini</strong>.
            </p>
            <hr />
            <div>
                <h3>Contents</h3>
                    ...
                    {toc.map((item) => {
                        ...
                        return (
                            <li key={target} className="underline text-primary hover:text-secondary">
                                <Link className="link-hash" href={`#${target}`} onClick={(e) => { e.preventDefault(); document.getElementById(target)?.scrollIntoView({ behavior: "smooth", block: "start" }); }}>{item.label}</Link>
                                {item.children && (
                                    <ul>
                                        {item.children.map((child) => {
                                            const childTarget = child.label.toLowerCase().replace(/\s+/g, "-");
                                            return (
                                                <li key={childTarget} className="underline text-primary hover:text-secondary">
                                                    <Link className="link-hash" href={`#${childTarget}`} onClick={(e) => { e.preventDefault(); document.getElementById(childTarget)?.scrollIntoView({ behavior: "smooth", block: "start" }); }}>{child.label}</Link>
                                                </li>
                                            );
                                        })}
                                    </ul>
            ...
            <DocSection
                ...
                conclusion={<p>Click <strong>Run Flybox</strong>. Progress updates will appear on the page and files will automatically download when done.</p>}
            >
