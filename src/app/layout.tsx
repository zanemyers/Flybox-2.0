...
const socialLinks = [
    ...
    { name: "Instagram", href: "https://instagram.com/rescueriver8", icon: FaInstagram },
];
                            ...
                            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                                ...
                                <div className="flex gap-4 md:gap-6">
                                    {socialLinks.map(({ name, href, icon: Icon }) => (
                                        <a key={name} href={href} target="_blank" rel="noopener noreferrer" className="text-base-content/70 transition-colors p-2 rounded-full hover:bg-primary hover:text-primary-content" aria-label={name}>
                                            <Icon size={20} />
                                        </a>
                                    ))}
                                </div>
                            ...
                            <div className="mt-8 pt-8 border-t border-base-300 flex flex-col md:flex-row justify-between items-center gap-4">
                                ...
                                <div className="flex gap-6 text-sm">
                                    {legalLinks.map(({ label, href }) => (
                                        <Link key={label} href={href} className="text-base-content/70 hover:text-base-content transition-colors">{label}</Link>
                                    ))}
                                </div>
                            </div>
                        </div>
